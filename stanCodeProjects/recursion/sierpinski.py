...
def sierpinski_triangle(order, length, upper_left_x, upper_left_y):
	...
	:return: A sierpinski triangle based on the order
	"""
	if order == 0:
		return
		...
		triangle.add_vertex((upper_left_x + length / 2, upper_left_y + length / 2 * 3**1/2))
		window.add(triangle)
		# upper left
		...
		# lower
		sierpinski_triangle(order-1, length/2, upper_left_x+length/4, upper_left_y+length/4*3**1/2)
